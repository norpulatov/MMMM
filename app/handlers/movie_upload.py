...
async def _finalize_movie(
    ...
    movie = await crud.create_movie(
        ...
        channel_chat_id=settings.channel_id,
        added_by=message.from_user.id,
    )
    deep_link = f"https://t.me/{settings.bot_username}?start=watch_{movie.movie_number}"
    text = f"🎬 #{movie.movie_number} - {movie.title}"
    if description:
        text += f"\n\n📝 {description}"
    ...
    await crud.update_movie_field(session, movie.movie_number, "channel_post_id", sent.message_id)
